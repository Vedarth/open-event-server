...
class EventOrgaSchema(Schema):
    ...
    def calc_total_tickets_count(obj):
        ...
        return db.session.query(func.sum(Ticket.quantity)).filter_by(event_id=obj.id).scalar()

    @staticmethod
    def calc_revenue(obj):
        """Returns total revenues of all completed orders for the given event"""
        revenue = db.session.query(func.sum(Order.revenue)).filter_by(event_id=obj.id, status='completed').scalar()
        if revenue is None:
            revenue = 0
        return revenue
